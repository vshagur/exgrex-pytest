...
@test "check output errors (syntax error, no mode, failfast=True)" {
  ...
  @test "check output errors (fixture error, mode=line, failfast=False)" {
  # check output
  PART_ID="DBCE"
  # delete the contents of the report file
  echo "" > $REPORT
  ...
  [ "$status" = 0 ]
  # check traceback
  run bash -c "grep -E \".*@pytest.fixture*\" $REPORT"
  [ "$status" = 0 ]
  run bash -c "grep -E \".*ests/test_example.py:13: ZeroDivisionError*\" $REPORT"
  [ "$status" = 0 ]
  run bash -c "grep -E \".*E       ZeroDivisionError: division by zero*\" $REPORT"
  [ "$status" = 0 ]
  # check resume
  run bash -c "grep -E \".*Not passed. Try again.*\" $REPORT"
  [ "$status" = 0 ]
